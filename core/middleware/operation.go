...
func OperationLog() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		isCore := strings.Contains(c.Request.URL.Path, "/api/v2/core")
		if isCore {
		...
		var res response
		_ = json.Unmarshal(bodyData, &res)
		if !isCore && (len(res.LogZH) == 0 || len(res.LogEN) == 0) {
			c.Next()
			return
		}
		if res.Code == 200 {
		...
		if len(res.LogZH) != 0 {
			record.DetailZH = res.LogZH
		}
		if len(res.LogEN) != 0 {
			record.DetailEN = res.LogEN
		}
		latency := time.Since(now)
		record.Latency = latency
